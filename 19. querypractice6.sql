...
-- TB_EMP, TB_SAL_HIS
-- 2020년 5월 기준 평균이상의 급여를 받고 있는 직원들의 리스트를 출력하시오.
...
SELECT A.EMP_NO , COUNT(CERTI_CD) 
FROM TB_EMP_CERTI A
	,(
		SELECT A.EMP_NO , COUNT(*) CNT  
		FROM TB_EMP_CERTI A 
...
SELECT A.EMP_NO 
		...
		,A.CERTI_CD 
		,(SELECT M.CERTI_NM FROM TB_CERTI M WHERE A.CERTI_CD = M.CERTI_CD) AS CERTI_NM
FROM TB_EMP_CERTI A
WHERE A.CERTI_CD IN 
(
	SELECT K.CERTI_CD 
...
SELECT B.EMP_NO
		...
		,B.CERTI_CD
		,(SELECT M.CERTI_NM FROM TB_CERTI M WHERE B.CERTI_CD = M.CERTI_CD) AS CERTI_NM
FROM 
(
	SELECT K.CERTI_CD 
	FROM EZEN.TB_CERTI K
